...
(    
    ...
    output    logic      q_async_rising_clk_rising_reset_o,     // -> Q output from flop using synchronous reset
    output    logic      q_async_rising_clk_falling_reset_o,    // -> Q output from flop using synchronous reset
    output    logic      q_async_falling_clk_falling_reset_o,   // -> Q output from flop using synchronous reset
    output    logic      q_async_falling_clk_rising_reset_o,    // -> Q output from flop using synchronous reset
    output    logic      q_sync_reset_o                         // -> Q output from flop using asynchrnoous reset
);
...
// | `q_sync_reset_o`                      | `posedge clk`                  | **Synchronous**  | High         | Rising     |
    ...
    always_ff@(posedge clk or posedge reset) // Asyc reset
        if (reset == 1'b1) begin // active high reset
            q_async_rising_clk_rising_reset_o <= 1'b0;
        end
        else begin 
            q_async_rising_clk_rising_reset_o <= d_i;
        end

    always_ff@(posedge clk or negedge reset) // Async reset
        if (reset == 1'b0) begin // active low reset
            q_async_rising_clk_falling_reset_o <= 1'b0;
        end
        else begin 
            q_async_rising_clk_falling_reset_o <= d_i;
        end

    always_ff@(negedge clk or negedge reset) // Async reset
        if (reset == 1'b0) begin // active low reset
            q_async_falling_clk_falling_reset_o <= 1'b0;
        end
        else begin 
            q_async_falling_clk_falling_reset_o <= d_i;
        end

    always_ff@(negedge clk or posedge reset) // Async reset
        if (reset == 1'b1) begin // active high reset
            q_async_falling_clk_rising_reset_o <= 1'b0;
        end
        else begin 
            q_async_falling_clk_rising_reset_o <= d_i;
        end
